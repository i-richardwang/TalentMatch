...
class ResumeSearchStrategyGenerator:
    """Resume search strategy generator class for generating overall search strategies"""
    ...
    async def generate_resume_search_strategy(
        ...
        Returns:
            List[Dict[str, float]]: List of dictionaries containing collection names and relevance scores

        Raises:
            ValueError: If refined query is empty
        """
        if not refined_query:
            raise ValueError("Refined query cannot be empty. Unable to generate search strategy.")

        session_id = session_id or str(uuid.uuid4())
        config = create_langfuse_config(
            session_id=session_id,
            run_name="generate_resume_search_strategy",
            task_name="resume_recommendation"
        )
        
        search_strategy_result = await self.resume_search_strategy_chain.ainvoke(
            {"refined_query": refined_query}, config=config
        )
        ...
        logger.info("Initial resume matching strategy generated")

        return collection_relevances


class CollectionSearchStrategyGenerator:
    """Collection-specific search strategy generator class"""
    ...
    async def generate_single_collection_strategy(
        ...
        """Generate search strategy for a single collection"""
        async with semaphore:
            ...
            config = create_langfuse_config(
                session_id=session_id,
                run_name=f"generate_collection_strategy_{collection_name}",
                task_name="resume_recommendation"
            )
    ...
    async def generate_collection_search_strategy(
        ...
        """
        Generate detailed search strategies for each relevant collection in parallel.

        Args:
            refined_query (str): Refined user query
            collection_relevances (List[Dict[str, float]]): List of collection relevances
            session_id (Optional[str]): Session ID

        Returns:
            Dict[str, CollectionSearchStrategy]: Search strategy for each collection

        Raises:
            ValueError: If refined query or collection relevance list is empty
        """
        if not refined_query or not collection_relevances:
            raise ValueError(
                "Refined query or collection relevance list cannot be empty. Unable to generate collection search strategy."
            )
        ...
        logger.info("Detailed resume search strategy formulation completed")
